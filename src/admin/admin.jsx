...
export default function AdminPanel() {
  ...
  useEffect(() => {
    const fetchRevenue = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8000/money', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  useEffect(() => {
    const fetchCartCount = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8000/count_cart', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (response.ok) {
          const data = await response.json();
          // FIX: Extract the count value properly
          const count = typeof data === 'object' ? (data.count || data.cart_count || 0) : data;
          setCartCount(count);
        } else {
  ...
  useEffect(() => {
    const fetchProductCount = async () => {
      try {
        const response = await fetch('http://127.0.0.1:8000/count_products', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
        ...
        if (response.ok) {
          const data = await response.json();
          // FIX: Extract the count value properly
          const count = typeof data === 'object' ? (data.count || data.product_count || 0) : data;
  ...
  const stats = [
    { label: 'Total Users', value: loading ? '...' : userCount.toLocaleString(), change: '+12.5%', positive: true , go :"/admin"},
    { label: 'Money', value: revenueLoading ? '...' : `$${totalRevenue.toLocaleString(undefined, {minimumFractionDigits: 2, maximumFractionDigits: 2})}`, change: '+8.2%', positive: true , go :"/admin"},
    { label: 'Count Products', value: productLoading ? '...' : productCount.toLocaleString(), change: '+15.3%', positive: true , go :"/Product"},
    { label: 'Count Cart', value: cartLoading ? '...' : cartCount.toLocaleString(), change: '-3.1%', positive: true , go :"/admin"},
  ];
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        {/* Subtle grid pattern - FIX: Added key prop */}
