...
const [isLoading, setIsLoading] = useState(false);
  ...
  const handleLogin = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("https://kenzy-api.usif.spaceseif.me/register", {
     ...
     <input
            ...
            >
              {isLoading ? (
               ...
               <br />
            <p> <span className='text-gray-400'>you already have an account?</span> <Link to="/login" className="text-blue-500 hover:underline">Sign in</Link></p>
      </div>
          </div>
                    </div>
          </div>
